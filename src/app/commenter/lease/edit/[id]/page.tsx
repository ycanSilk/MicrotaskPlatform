...
export default function EditAccountRentalPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 在实际应用中，这里会调用API提交数据
      console.log('更新的账号出租任务数据:', formData);
      
      // 模拟API请求延迟
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // 提交成功后返回账号出租详情页面
      router.push(`/commenter/lease/${id}`);
    } catch (err) {
      console.error('更新账号出租任务时出错:', err);
      setError('更新失败，请稍后重试');
    } finally {
  ...
  if (error) {
    return (
      <div className="p-4">
        ...
        <Button onClick={() => router.push('/commenter/lease')} variant="secondary">
          返回列表
        </Button>
      </div>
    );
    ...
    <div className="p-4">
      ...
      <div className="mb-6">
        <div className="flex items-center mb-2">
          ...
          <h1 className="text-2xl font-bold text-gray-800">编辑账号出租任务</h1>
        </div>
        <p className="text-gray-500">修改您的账号出租任务信息</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center">
              ...
              />
              <label htmlFor="verificationRequired" className="ml-2 block text-sm text-gray-700 cursor-pointer">
                需要身份验证
              </label>
            </div>
      ...
      <Card>
        <div className="p-4">
          <h2 className="text-lg font-bold text-gray-800 mb-3">编辑须知</h2>
          <ul className="list-disc list-inside space-y-2 text-gray-700 text-sm">
            <li>修改账号信息后，任务将重新进入审核状态</li>
            <li>租金调整可能会影响已有的预约订单，请谨慎操作</li>
            <li>有效期可以延长，但不能缩短至小于当前日期</li>
            <li>如遇到技术问题，请联系平台客服</li>
          </ul>
        </div>
      </Card>
    </div>
